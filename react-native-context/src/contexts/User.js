...
//기본 값으로 user 객체와, dispatch 함수를 설정
//user는 사용자의 정보, dispath는 이름을 업데이트하는 함수로 사용할 예정
...
const UserProvider = ({ children }) => {
    const [name, setName] = useState('Jisu')

    //value 객체에는 현재 user의 상태와 상태를 업데이트할 함수 dispatch를 포함
    //dispatch에다 setName함수를 전달하여 하위 컴포넌트들도 이름을 업데이트 할수 있도록 한다.
